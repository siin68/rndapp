      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {loading ? (
             Array.from({ length: 4 }).map((_, i) => (
                <Card key={i} className="border-0 bg-white rounded-3xl overflow-hidden h-[400px] animate-pulse shadow-sm">
            ...
            openEvents.map((event) => {
              ...
              const displayCount = shouldTruncate ? showMax - 1 : totalHobbies;
                  ...
                  <div className="relative aspect-[4/5] w-full overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex flex-wrap gap-1.5 max-w-[90%]">
                      {displayHobbies.map((hid) => {
                        ...
                        return (
                          <span key={hid} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-white/95 backdrop-blur-sm text-gray-800 text-[10px] font-bold shadow-sm">
                            <span>{h?.emoji}</span>
                  ...
                  <CardContent className="p-5 flex flex-col gap-3 relative">
                    ...
                    <div className="grid grid-cols-2 gap-y-2 text-xs text-gray-500 font-medium mt-auto">
                       ...
                       <div className="flex items-center gap-1.5 justify-end">
                         ...
                         <span className="text-gray-700 font-bold">{spotsLeft} spots left</span>
                       </div>
                       <div className="flex items-center gap-1.5 col-span-2">
                         <MapPinIcon className="w-3.5 h-3.5 text-indigo-400" />
                         <span className="truncate">{location?.name || "Online"}</span>
                       </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        {!loading && openEvents.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20">
             ...
             <p className="text-gray-500 font-medium">No active invites found.</p>
             <Button variant="link" onClick={() => router.push("/dashboard/create-invite")} className="text-rose-600 font-bold">
               Create one?
             </Button>
          </div>
        )}
